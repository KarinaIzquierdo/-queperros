...
@section('title', 'Aprobación de Servicios')
        ...
        <div class="approval-card approval-card--{{ $approval->estado ?? 'unknown' }}" data-status="{{ $approval->estado ?? 'unknown' }}">
            ...
            <div class="approval-actions">
                ...
                @if ($approval->estado === 'aprobado')
                    <button class="btn btn--primary btn--sm" onclick="openPaymentModal({{ $approval->id }})">
                        <i class="bi bi-cash"></i>
                        Confirmar Pago
                    </button>
                @endif
...
<div class="modal" id="approveModal" style="display: none;">
    ...
    <div class="modal-card">
        ...
        <form method="POST" action="{{ route('entrenador.approvals.approve', '__approval__') }}" id="approveForm">
            @csrf
            <div class="modal-body">
                <div class="form-group">
                    <label for="approve_notes">Notas del entrenador (opcional)</label>
...
<div class="modal" id="rejectModal" style="display: none;">
    ...
    <div class="modal-card">
        ...
        <form method="POST" action="{{ route('entrenador.approvals.reject', '__approval__') }}" id="rejectForm">
            @csrf
            <div class="modal-body">
                <div class="form-group">
...
<div class="modal" id="paymentModal" style="display: none;">
    ...
    <div class="modal-card">
        ...
        <form method="POST" action="{{ route('entrenador.approvals.confirmPayment', '__approval__') }}" id="paymentForm">
            @csrf
            <div class="modal-body">
...
<style>
/* Estilos específicos para el panel de aprobaciones del entrenador */
.page-header {
    margin-bottom: 2rem;
}
...
.approval-card--pending { border-left-color: #f59e0b; }
.approval-card--approved { border-left-color: #10b981; }
.approval-card--rejected { border-left-color: #ef4444; }
.approval-card--paid { border-left-color: #8b5cf6; }
...
.status-badge--paid { background: #ede9fe; color: #5b21b6; }
